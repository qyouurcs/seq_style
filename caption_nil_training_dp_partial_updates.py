...
if __name__ == '__main__':
    ...
    save_fn = 'lstm_coco_trained_{}_partial.pkl'.format(LEARNING_RATE)
    ...
    emb_params = lasagne.layers.get_all_params(l_cnn_embedding, trainable=True)
    ...
    emb_grads = T.grad(loss, emb_params)
    emb_grads = [T.clip(g, -5, 5) for g in emb_grads]
    emb_grads, norm = lasagne.updates.total_norm_constraint(
        emb_grads, MAX_GRAD_NORM, return_norm=True)
    
    updates = lasagne.updates.adam(emb_grads, emb_params, learning_rate=LEARNING_RATE)
    
    f_train = theano.function([x_cnn_sym, x_sentence_sym, mask_sym, y_sentence_sym],
                              [loss, norm],
                              no_default_updates = True, # do not update the parameters except for those
                              # In the updates list.
                              updates=updates
                             )
