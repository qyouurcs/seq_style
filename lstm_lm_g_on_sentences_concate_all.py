...
def main():
    ...
    save_fn = os.path.join(save_dir, os.path.basename(sys.argv[1]) + '_' + optim  + '_' + h_size_str + '_concat_all.pkl')
    ...
    with open(val_fn) as fid:
        for i,aline in enumerate(fid):
            ...
            if aline[-1] != '.':
                aline = aline + ' .'
            val_str.append(aline)
    val_str = ' '.join(val_str)
    val_str = val_str.split(' ')
    
    train_range = range(len(train_str))
    def batch_train(batch_size = BATCH_SIZE):
        random.shuffle(train_range)
        batch = train_range[0:batch_size]
        k = batch_size
        for i,b in enumerate(batch):
            if b + MAX_SEQ_LENGTH > len(train_str):
                while train_range[k] + MAX_SEQ_LENGTH > len(train_str):
                    k = k + 1
                batch[i] = train_range[k]
                k += 1
        ...
        for i, sent in enumerate(batch):
            tokens = train_str[batch[i]:batch[i] + MAX_SEQ_LENGTH]
    ...
    val_range = range(len(val_str))
    def batch_val(batch_size = BATCH_SIZE):
        random.shuffle(val_range)
        batch = val_range[0:batch_size]
        k = batch_size
        for i,b in enumerate(batch):
            if b + MAX_SEQ_LENGTH > len(val_str):
                while val_range[k] + MAX_SEQ_LENGTH > len(val_str):
                    k = k + 1
                batch[i] = val_range[k]
                k += 1
        ...
        for i, sent in enumerate(batch):
            tokens = val_str[batch[i]:batch[i] + MAX_SEQ_LENGTH]
    ...
    data_size = len(train_str) / MAX_SEQ_LENGTH
